...
static void sinus  (GimpDrawable     *drawable);
...
query (void)
{
  ...
  gimp_plugin_menu_register ("plug-in-sinuss",
                             "<Image>/Filters/Math");
}
...
/*          guchar pixel[9][4];   */
        ...
        */
          /* For each layer, compute the sinus */
          for (k = 0; k < channels; k++)
            {
                double angle = pixel[k], max=255;
                angle = M_2_PI * (angle / max ) ;
                output[k] = (guchar)(255*(1+sin(angle))/2);
            }
